...
global h_directory h_run h_pause h_stop n_bats h_bat h_feeders h_session h_d_rew h_Name h_Exp
...
bhv_data.fields = {'P1','P2','P3','P4','R1','R2','R3','R4','S1','S2','S3','S4','bat_ID','activated_feeder','eventtime','outcome','rew_decr','session'};
bhv_data.trials = [];
...
%                 disp(sprintf('%d: Enabled feeder%d',y,fd)) 
            ...
            if is_enabled                                                                               %If enabled ->
                ...
                if fd_state(active_feeder) && fd_enabled(active_feeder)                                       %if active feeder is on
                    ...
                    if pf(active_feeder) >= (rand*100)                                                      %If probability allows
                        ...
                        bhv_data.trials(t_counter,:) = [pf', cell2mat(h_feeders.Data(:,5))',cell2mat(h_feeders.Data(:,6))', who_triggered, active_feeder, eventtime, 1, d_rew, session];
                        save(bhv_data.filename,'bhv_data');
                    else
                        %Save behavioral file with 0 outcome
                        t_counter = t_counter + 1;  eventtime = toc(start_time);
                        bhv_data.trials(t_counter,:) = [pf', cell2mat(h_feeders.Data(:,5))', cell2mat(h_feeders.Data(:,6))', who_triggered, active_feeder, eventtime, 0, d_rew, session];
                        save(bhv_data.filename,'bhv_data');
                    end
                end
            end
    ...
    if h_pause.Value && ~h_stop.Value
        ...
        if ~pause_state
            t_counter = t_counter + 1;  eventtime = toc(start_time);
            bhv_data.trials(t_counter,:) = [pf', cell2mat(h_feeders.Data(:,5))',cell2mat(h_feeders.Data(:,6))', -1, 0, eventtime, 0, d_rew, session]; % -1 in the 9th column when task paused
            save(bhv_data.filename,'bhv_data');
    ...
    if ~h_pause.Value && ~h_stop.Value
        ...
        t_counter = t_counter + 1;  eventtime = toc(start_time);
        session = str2num(h_session.String); % current session
        bhv_data.trials(t_counter,:) = [pf', cell2mat(h_feeders.Data(:,5))', cell2mat(h_feeders.Data(:,6))', -2, 0, eventtime, 0, d_rew, session]; % -2 in the 9th column when task resumed
        save(bhv_data.filename,'bhv_data');
