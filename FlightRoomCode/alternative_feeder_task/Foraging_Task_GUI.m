...
global h_directory h_run h_pause h_stop n_bats h_bat h_feeders h_session h_Name h_Exp h_Alt h_Alt_1 h_Alt_2 h_Alt_now
...
set(h_fig, 'units','normalized','Position',[0.05 0.2 0.5 0.6])
% [65 143 700 500]
...
%Feeder alternation
h_alt = uicontrol(h_fig,'Style','text','String','Feeder alternation','units','normalized',...
   'Position',[.01 .54 .23 .05],'fontsize',9,'fontweight','b'); 
h_Alt = uicontrol(h_fig,'Style','edit','String','20','units','normalized',...
    'Position',[.01 .50 .23 .05],'fontsize',9,'fontweight','b');

h_alt_now = uicontrol(h_fig,'Style','text','String','Current count','units','normalized',...
   'Position',[.26 .54 .23 .05],'fontsize',9,'fontweight','b'); 
h_Alt_now = uicontrol(h_fig,'Style','edit','String','0','units','normalized',...
    'Position',[.26 .50 .23 .05],'fontsize',9,'fontweight','b');

h_alt_1 = uicontrol(h_fig,'Style','text','String','Feeder to switch','units','normalized',...
   'Position',[.51 .54 .23 .05],'fontsize',9,'fontweight','b'); 
h_Alt_1 = uicontrol(h_fig,'Style','edit','String','1','units','normalized',...
    'Position',[.51 .50 .23 .05],'fontsize',9,'fontweight','b');

h_alt_2 = uicontrol(h_fig,'Style','text','String','Feeder to switch','units','normalized',...
   'Position',[.76 .54 .23 .05],'fontsize',9,'fontweight','b'); 
h_Alt_2 = uicontrol(h_fig,'Style','edit','String','2','units','normalized',...
    'Position',[.76 .50 .23 .05],'fontsize',9,'fontweight','b');


%Feeders table
% sAlign = uistyle("HorizontalAlignment","center");
fed_table = table2cell(table((1:4)',[true(2,1);false(2,1)],true(4,1),[100;0;0;0],0.2*ones(4,1),zeros(4,1)));
h_feeders = uitable(h_fig,'Data',fed_table,'ColumnName',{'Feeder ID'; 'Enabled'; 'State'; 'Probability'; 'Duration'; 'Reward'},'units','normalized',...
    'Position',[.11 .01 .38 .44],'fontsize',9,'fontweight','b','ColumnEditable',true,'ColumnWidth',{60,50,40,65,55,60});
...
h_bat = uitable(h_fig,'Data',bat_table,'ColumnName',{'Tag'; 'Bat ID'; 'Enabled'; 'Trials'; 'Correct'; 'Ephys'; 'Region'},'units','normalized',...
    'Position',[.50 .01 .49 .44],'fontsize',9,'fontweight','b','ColumnEditable',true,'ColumnWidth',{50,45,48,60,60,45,60});
...
h_feed4 = uicontrol(h_fig,'Style','pushbutton','String','Feed4','units','normalized',...
    'Position',[.01 .1 .09 .05],'Callback',@feed_button,'fontsize',10,'fontweight','b','tag','run');

% Run motors to change syringes
h_reload = uicontrol(h_fig,'Style','togglebutton','String','Reload','units','normalized',...
    'Position',[.01 .01 .09 .05],'Callback',@run_feeders_in_task,'fontsize',10,'fontweight','b','tag','run');
