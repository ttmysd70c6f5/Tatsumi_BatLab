function extract_lfp_from_tetrode()
...
% Modifed from the script written by Angelo Forli.

behavior_dir = dir(fullfile(cd,'Ext_Behavior*','Extracted_Behavior*'));
load(fullfile(behavior_dir.folder,behavior_dir.name),'r','v_abs','a_abs','t','bflying','batdate','n_tags','T'); % Behavioral variables (velocity, acceleration, etc...)

ephys_dir = dir(fullfile(cd,'Ext_Ephys*'));
for bat = 1:length(ephys_dir)
    TTL_dir = dir(fullfile(ephys_dir(bat).folder,ephys_dir(bat).name,'TTL_timestamps.mat'));
    load(fullfile(TTL_dir.folder,TTL_dir.name),'TTL_timestamps'); % Reference Timestamps for alignment
    imp_bat_dir = dir(fullfile(ephys_dir(bat).folder,ephys_dir(bat).name,'imp_bat.mat'));
    load(fullfile(imp_bat_dir.folder,imp_bat_dir.name),'imp_bat'); % Tag of the implanted bat

    %=== Get the name of the folder for saving the data
    analysis_directory=fullfile(ephys_dir(bat).folder,ephys_dir(bat).name,['CSC_Analysis_',batdate,]);
    ...
    Freq_band = [0.5 150];          % Frequency b and for filtering the signal
    ...
    for TT = 1:4
        ...
        for ch = 1:4
            ...
            % load(ephys_data(bat).csc(CSC(ch)).filepath,'Estimated_channelFS_Transceiver','AD_count_int16','AD_count_to_uV_factor','Timestamps_of_first_samples_usec');
            CSC_file = dir(fullfile(ephys_dir(bat).folder,ephys_dir(bat).name,['*CSC' num2str(CSC(ch)),'.mat']));
            load(fullfile(CSC_file.folder,CSC_file.name),'Estimated_channelFS_Transceiver','AD_count_int16','AD_count_to_uV_factor','Timestamps_of_first_samples_usec');
        ...
        if mean(channel_pairs(bad_flag,3))/mean(channel_pairs(~bad_flag,3))>5 && nnz(bad_flag)==3 
            ...
            while ~all(any(channel_pairs(bad_flag,1:2) == i,2)) && i<=4
                i=i+1; % channel i is not a bad channel
            end
        ...
        LFP_temp = interp1(t_fly,bandpass(V_fly,Freq_band,Fs_CSC),t_1KHz);   % 0.85 steepness, 60 dB
