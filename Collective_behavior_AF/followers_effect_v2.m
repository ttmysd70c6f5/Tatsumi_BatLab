...
for i=1:n_tags
    nexttile(); hold on
    for cl = 1:2
        ifi = ifi_log_AllSessions{1,i}; ifi_idx = IFI_cluster_all{1,i};
...
for ss=1:length(sessions)
    ...
    load(fullfile(sessions(ss).folder, 'bouts.mat'),'bout','bout_num','seg_alg')
    ...
    for i = 1:n_tags
        ...
        area(t,bout(:,i)*3,'FaceAlpha',0.3,'LineStyle','-','FaceColor',bat_clr(i,:),'EdgeColor',bat_clr(i,:))
        hold off
