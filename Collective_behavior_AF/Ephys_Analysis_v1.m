...
load('Ext_Ephys*/imp_bat.mat'); % imp_bat: number of implanted bats
...
load(fullfile(BHV2_file.folder,BHV2_file.name));

% bpos_file = dir(fullfile(fileparts(TTlFile.folder),'Ext_Beh*','bpos*'));
% load(fullfile(bpos_file.folder,bpos_file.name));

bout_file = dir(fullfile(fileparts(TTlFile.folder),'Ext_Beh*','bouts*'));
load(fullfile(bout_file.folder,bout_file.name));

dirStr = strsplit(fileparts(TTlFile.folder),'\');
DatasetName = dirStr{end-1};
RecDate = dirStr{end};
