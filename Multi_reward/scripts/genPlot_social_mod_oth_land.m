function genPlot_social_mod_oth_land(BhvData,Flights,TrjCluster,spikeData,mtData,FigDir)
...
for bbb = [1:bb-1,bb+1:n_bat]
    ...
    t_oth_land_sec = Flights(bbb).t_flight_sec(:,2); % Time of landings of the target bat
    d_oth_land = Flights(bbb).dist_oth(:,bb); % distance to the ephys bat at landings of the target bat
    is_social = d_oth_land < th_social;
    ...
    is_include2 = t_oth_land_sec > t_ephys_start & t_oth_land_sec < t_ephys_end;
    
    %% Distribution of acceleration for social/non-social flights
    ac = cell2mat(cellfun(@(x) x(1,bb), Flights(bbb).a_land, 'UniformOutput',false)); % acceleration (x) of self at landing
    ...
    mod_other_land(bb).clu_id = TrjCluster(bbb).clu_list;
    mod_other_land(bb).id = unit_ids;
    mod_other_land(bb).depth = unit_depths;
    mod_other_land(bb).region = regions;
    mod_other_land(bb).fr = frs;
    ...
    for clu = 1:n_trjclu 
        ...
        if n_flight_clu < 5 % minimum number of flights
            ...
            t_social = t_oth_land_sec(social_adjusted == clu_id);
            t_asocial = t_oth_land_sec(asocial_adjusted == clu_id);
            ...
            % for unit = 34:34 % run for all units
                ...
                % Texts
                text(x_fd1-0.1, y_fd1, 'Non-social', 'fontsize', 14,'HorizontalAlignment','center','Rotation',90)
                text(x_fd2-0.1, y_fd2, 'Social', 'fontsize', 14,'HorizontalAlignment','center','Rotation',90)
            ...
            mod_other_land(bb).t_opt(clu).t_opt = t_opt_bin; % t with most significant modulation
            mod_other_land(bb).p_ctest(clu).p_ctest = p_ctest;
            mod_other_land(bb).p_glm(clu).p_glm = p_glm;
        end
    end
    mod_other_land(bb).gen_fig = gen_fig;
end
saveFile = fullfile(FigDir,'social_modulation_other_land.mat');
