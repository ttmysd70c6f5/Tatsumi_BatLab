function genPlot_social_mod_self_land(BhvData,Flights,TrjCluster,spikeData,mtData,FigDir)
...
bnames = mtData.bname;
t_self_land_sec = Flights(bb).t_flight_sec(:,2); % time of landings
n_flight = Flights(bb).n_flight; % number of flights
d_oth_land = Flights(bb).dist_oth; % distance to the ephys bat at landings of the target bat
...
is_include = t_self_land_sec > t(1)-t_margin(1) & t_self_land_sec < t(end)-t_margin(2);
...
is_include2 = t_self_land_sec > t_ephys_start & t_self_land_sec < t_ephys_end;
...
n_trjclu = TrjCluster(bb).n_clu;
gen_fig = true(1,n_trjclu);
for clu = 1:n_trjclu 
    clu_id = TrjCluster(bb).clu_list(clu);
    clu_now = TrjCluster(bb).clu_idx == clu_id; % logical vector for the current cluster

    % for bbb = [1:bb-1,bb+1:n_bat]
    % bbb = 8;
    
    % Identify social flights
    is_social = any(d_oth_land(:,[1:bb-1,bb+1:end]) < th_social,2);

    %% Plot firing curve and raster plots for each cluster
    mod_self_land_anyBat.clu_id = TrjCluster(bb).clu_list;
    mod_self_land_anyBat.id = unit_ids;
    mod_self_land_anyBat.depth = unit_depths;
    mod_self_land_anyBat.region = regions;
    mod_self_land_anyBat.fr = frs;
    
    % 1. Check exclusion criteria
    % n_flight_clu = sum(TrjCluster(bb).clu_idx == clu_id);
    % number of flights
    n1 = sum(is_social&is_include&is_include2&clu_now);
    n2 = sum(~is_social&is_include&is_include2&clu_now); % number of flights for each category (after adjustment)
        % n_event = n1 + n2; % total length of flights

    if n1 < 5 || n2 < 5 % minimum number of flights
        gen_fig(clu) = false; % false if the # of flights is insufficient
    else
        % social/asocial flights after adjustment
        t_social = t_self_land_sec(is_social&is_include&is_include2&clu_now);
        t_asocial = t_self_land_sec(~is_social&is_include&is_include2&clu_now);
        ...
        % for unit = 34:34 % run for all units
            ...
            T_vars = table(social_glm,spcnt_glm);
            ...
            title_str = sprintf('%s: Unit%d, %s, %dum, %.1fHz (Social: %.1fHz, Non-social: %.1fHz)',mtData.recName,unit_id,region,depth,fr,pefr_social,pefr_asocial);
            ...
            xlim(t_margin)
            ylim([0,n1+n2])
            xlabel('Time from self landings (sec)')
            ...
            %%% Save figure
            saveDir = fullfile(FigDir,sprintf('ephys%s_%s',mtData.bname_ephys,mtData.trjName),region,'anyBat',sprintf('Clu%d',clu_id));
            if isempty(dir(saveDir)); mkdir(saveDir); end
            saveas(fig,fullfile(saveDir,sprintf('Unit%d.jpg',unit_id)))
            close(fig)
        end
        mod_self_land_anyBat.t_opt(clu).t_opt = t_opt_bin; % t with most significant modulation
        mod_self_land_anyBat.p_ctest(clu).p_ctest = p_ctest;
        mod_self_land_anyBat.p_glm(clu).p_glm = p_glm;
    end
    mod_self_land_anyBat.gen_fig = gen_fig;
end
saveFile = fullfile(FigDir,'social_modulation_self_land.mat');
save(saveFile,'mod_self_land_anyBat');
