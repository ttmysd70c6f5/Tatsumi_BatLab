function plot_reward_modulation_land(bname_ephys,trjName,Flights,TrjCluster,spikeData,mtData,Layers)
...
for clu = 1:n_cluster
    ...
    if n_trj >= 3 && cluster_list(clu) ~= -1 % Plot only the cluster with more than 2 trajectories
        ...
        if sum(any([idx_r1,idx_r2],2)) ~= 0
            t_onset_sec_r1 = t_land_sec(idx_r1,1);
            t_end_sec_r1 = t_land_sec(idx_r1,1);
        
            t_onset_sec_r2 = t_land_sec(idx_r2,1);
            t_end_sec_r2 = t_land_sec(idx_r2,1);
        
            FigDir = fullfile(pwd,'analysis','figure','reward_modulation_landing', ...
