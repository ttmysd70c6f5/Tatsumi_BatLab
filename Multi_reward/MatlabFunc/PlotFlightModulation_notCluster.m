function PlotFlightModulation_notCluster(BhvData,SocialFlights,spikeData,target,FigDir)
% Common parameters
t_margin = [-1 1];
...
for bbb = 1:n_bats
    ...
    is_social = SocialFlights(bbb).social;
    a_obs = SocialFlights(bbb).a_abs_obs;
    t_flight = SocialFlights(bbb).t_sec;
    ...
    if n1 >= 5 && n2 >= 5 % Check exclusion criteria
        ...
        %     fr_all = [fr_all; {bname,unit_id,fr,0,p_ctest(unit),p_anova(unit),out_glm(unit).social,fr1(:,unit)}];
        % end
        % fr_field = {'bname','unit_id','fr','clu_id','p_ctest','p_anova','glm_social','pefr'};
        ...
        t_fig = t_margin(1):w_step:t_margin(2); % time vector for figure
            ...
            %%% Save figure
            saveDir = fullfile(FigDir,bname,'all',region);
