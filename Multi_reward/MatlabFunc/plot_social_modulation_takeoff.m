function plot_social_modulation_takeoff(bname_ephys,trjName,Flights,TrjCluster,spikeData,mtData,Layers)
% Specify the ephys bat
    ...
    n_flight = length(t_flight_sec); % number of landings
    ...
    % for clu = 29:29
        ...
        if n_trj >= 3 && cluster_list(clu) ~= -1
            ...
            t_end_sec = t_flight_sec(idx,1);
            for k = 1:n_bats
                if k == 1 % plot for nearest neighbor bat
                    oth_dist = Flights(bb).closest_oth_dist(idx);  % distance to the nearest neighbor bat at takeoff
                    FigDir = fullfile(pwd,'analysis','figure','social_modulation_takeoff', ...
