function [data] = readTrodesExtractedDataFile_new(filename, varargin)  
...
%
%
% New feature added by TY on 09/21/2023:
% By specifying offset and batch_size, the specific part of data can be
% loaded.
% Input:
% offset -- the number of samples to skip
% batch_size -- the size of data to be loaded
...
%read in each field
% byteOffset = 0;
byteOffset = offset*bytesPerPacket;
for i = 1:length(fields)
    ...
    tmpData = fread(fid,[fields(i).columns,batch_size],[num2str(fields(i).columns),'*', fields(i).type,'=>',fields(i).type],skipBytes);
